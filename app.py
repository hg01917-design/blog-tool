...
SEO_PROMPTS_LIVING = {
    "naver": (
        ...
        "ㅠㅠ, ㅎㅎ 이모티콘 자연스럽게 1~2개\n\n"
        "[1인칭 체험담 vs 3인칭 정보형 - 주제별 자동 전환 필수]\n"
        "- 기본 모드: 1인칭 체험담 ('해봤더니', '신청해봤는데')\n"
        "- 단, 아래 주제는 반드시 3인칭 정보형으로 전환:\n"
        "  · 기초생활수급자, 차상위계층, 장애인, 한부모가정 등 대상자가 한정된 지원금\n"
        "  · 긴급복지, 의료급여, 생계급여 등 취약계층 전용 제도\n"
        "  → 이 경우: '나는 ~했어요' 절대 금지\n"
        "  → 대신: '해당되는 분들은 ~할 수 있어요', '대상자라면 ~가능해요'\n"
        "  → '찾아보니', '알아봤는데', '정리해봤어요' 식의 정보 전달자 시점 사용\n"
        "- 일반 절약팁(통신비, 전기세 등)은 기존 1인칭 체험담 유지\n\n"
        "[네이버 모바일 가독성 - 필수]\n"
        "- 한 문단은 2~3줄 이내로 짧게\n"
        "- 문단 사이 줄바꿈 자주 사용\n"
        "- 긴 내용은 잘게 쪼개서 나눠서 써\n"
...
SEO_PROMPTS_GOVERNMENT = {
    ...
    "naver": (
        ...
        "[중요: 3인칭 정보형 작성 필수]\n"
        "- 정부지원금 글은 반드시 3인칭 정보형으로 작성\n"
        "- '나는 ~했어요', '제가 신청해봤는데', '저도 받았는데' 등 1인칭 체험담 절대 금지\n"
        "- 특히 기초수급/차상위/장애인/한부모 등 대상 한정 지원금은\n"
        "  본인이 해당자인 것처럼 쓰면 안 됨\n"
        "- 올바른 표현: '해당되는 분들은 ~', '대상자라면 ~가능해요',\n"
        "  '찾아보니 ~', '알아봤는데 ~', '정리해봤어요'\n\n"
        "[네이버 모바일 가독성 - 필수]\n"
        "- 한 문단은 2~3문장 이내로 짧게\n"
        "- 문단 사이 줄바꿈 자주 사용\n"
